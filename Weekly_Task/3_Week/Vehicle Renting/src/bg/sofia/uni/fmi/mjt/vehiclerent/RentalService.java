...
public class RentalService {
    ...
    public void rentVehicle(Driver driver, Vehicle vehicle, LocalDateTime startOfRent) {
        if (driver == null || vehicle == null || startOfRent == null) {
            throw new IllegalArgumentException("Driver or vehicle or start date is null");
        }
        if (vehicle.isRented()) {
            throw new VehicleAlreadyRentedException("The vehicle is already rented by someone else or by the same driver!");
        }
    ...
    public double returnVehicle(Vehicle vehicle, LocalDateTime endOfRent) throws InvalidRentingPeriodException {
        if (vehicle == null || endOfRent == null) {
            throw new IllegalArgumentException("Vehicle or end date is null");
        }
        if (!vehicle.isRented()) {
            throw new VehicleNotRentedException("Can't return back because vehicle is not rented at all!");
        }
        double result = vehicle.calculateRentalPrice(vehicle.getStartRentTime(), endOfRent);
